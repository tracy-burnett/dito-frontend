...
export default {
  ...
  methods: {
    ...
    updateText() {
       ...
       this.instructions=this.patienceDiffPlus(this.original_text.split(this.spaced_by), this.latest_text.split(this.spaced_by))
      //  let indicestodelete = []
for (let i = this.instructions.lines.length - 1; i >= 0; i--) {
  // console.log(this.instructions.lines[i].aIndex)
  // console.log(this.instructions.lines[i].bIndex)
  if (this.instructions.lines[i].aIndex == this.instructions.lines[i].bIndex) {
